...
class FileTracker:
    ...
    def __init__(self, on_file_complete):
        ...
        self._blocks = {}          # file_hash -> {block_id: block_bytes}
    ...
    def handle_block_assembled(self, file_hash, block_id, total_blocks, block_bytes, contributing_receivers):
        with self._lock:
            if file_hash in self._completed_files:
                return  # already finished and handed off
        ...
        self._on_file_complete(file_hash=file_hash, total_blocks=total_blocks, file_bytes=ordered_bytes)